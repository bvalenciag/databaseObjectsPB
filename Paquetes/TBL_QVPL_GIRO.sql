prompt
prompt PACKAGE: TBL_QVPL_GIRO
prompt
CREATE OR REPLACE PACKAGE tbl_qvpl_giro IS
--
-- Reúne funciones y procedimientos directamente relacionados con el procedimiento de las cartas vpl Giros
--
-- #VERSION: 1000
--
-- History
...
TYPE ty_data_sebra IS RECORD(
    ...
    , cargo_carta               VARCHAR2(200)             --6  Cargo del destinatario de la carta
    , dire_dest_carta           VARCHAR2(200)             --7  Dirección destino de la carta
    , ciud_dest_carta           VARCHAR2(1000)            --8  Ciudad del destinatario de la carta
    , fax_dest_carta            VARCHAR2(200)             --9  Fax destinatario de la carta
    , tele_dest_carta           VARCHAR2(200)             --10 Telefeno del destinatario de la carta 
    , emai_dest_carta           VARCHAR2(1000)            --11 Correo Electronico del destinatario de la carta 
    , tipo_oper                 VARCHAR2(200)             --12 Tipo de operación (debito-egreso o credito ingreso)
    , valor_tras_comi_num       VARCHAR2(200)             --13 Valor del traslado mas comisión fija mas comision por millon en números 
    , valor_tras_comi_let       VARCHAR2(400)             --14 Valor del traslado mas comisión fija mas comision por millon en letras  
    , tipo_cuen                 VARCHAR2(200)             --15 Tipo de cuenta a la cual se registra el debito y el credito
    , cod_cuenta                VARCHAR2(200)             --16 Cuenta bancaria del aporte
    , cod_cuent_banc_empr       VARCHAR2(200)             --17 Cuenta bancaria asociada a la empresa y banco, diferente a la relacionada en el traslado
    , vari_5                    VARCHAR2(1000)            --18 Variable 5  
    , nomb_empr                 VARCHAR2(200)             --19 Nombre de la empresa que hace el traslado
    , empr_nit                  VARCHAR2(200)             --20 Nit de la empresa que hace el traslado
    , concep_cud                VARCHAR2(200)             --21 Concepto CUD 
    , cod_cuen_cud              VARCHAR2(200)             --22 Código cuenta CUD
    , portafolio                VARCHAR2(200)             --23 Portafolio 
    , vari_15                   VARCHAR2(1000)            --24 Variable 15
    , vari_2                    VARCHAR2(1000)            --25 Variable 2
    , vari_3                    VARCHAR2(1000)            --26 Variable 3
    , confir_carta_nomb         VARCHAR2(1000)            --27 Nombre confirmación
    , confir_carta_tele         VARCHAR2(1000)            --28 Telefono confirmación
    , confir_carta_exte         VARCHAR2(1000)            --29 Extención confirmación
    , confir_sebra_nomb         VARCHAR2(1000)            --30 Nombre contacto sebra
    , confir_sebra_tele         VARCHAR2(1000)            --31 Telefono contacto sebra
    , confir_sebra_exte         VARCHAR2(1000)            --32 extensión contacto sebra
);
--
...
/
prompt
prompt PACKAGE BODY: tbl_qvpl_giro
prompt
--
CREATE OR REPLACE PACKAGE BODY tbl_qvpl_giro IS
--
-- #VERSION: 1000
--
FUNCTION ws_fn_acx_fill_sebra (p_traslado tbl_ttrasebra.tras_tras%TYPE
...
SELECT list_sigla                                                                                                                           ing_egr
             ...
             , DECODE(list_sigla, 'E', 'DEBITO', 'I', 'CREDITO')                                                                            tipo_oper
             , RTRIM(LTRIM(TO_CHAR((tras_valor+NVL(infb_comision,0)+NVL(infb_comision_m,0)),'L99G999G999G999G999G999G999D99MI','NLS_NUMERIC_CHARACTERS = '',.''NLS_CURRENCY = ''$ ''')))
                                                                                                                                            valor_tras_comi_num
             , tbl_qreporsebra.fn_numero_a_texto(round(tras_valor+NVL(infb_comision,0)+(NVL(infb_comision_m,0)),2))                         valor_tras_comi_let
             ...
             , tbl_qreporsebra.fn_valor_variable (5)                                                                                        vari_5
             , empr_descripcion                                                                                                             nomb_empr
             , empr_nit                                                                                                                     empr_nit
             , infb_concep                                                                                                                  concep_cud    
             , infb_cuen_cud_cod                                                                                                            cod_cuen_cud
             , infb_portafolio                                                                                                              portafolio
             , tbl_qreporsebra.fn_valor_variable (15)                                                                                       vari_15
             , tbl_qreporsebra.fn_valor_variable (2)                                                                                        vari_2
...
BEGIN
    ...
    vg_data_sebra(0).cargo_carta            := NULL;
    ...
    vg_data_sebra(0).tipo_oper              := NULL;
    ...
    vg_data_sebra(0).vari_15                := NULL;
    vg_data_sebra(0).vari_2                 := NULL;
    ...
    FOR i IN c_tras LOOP
    	...
    	vg_data_sebra(v_count).cargo_carta          := i.cargo_carta            ;--6  Cargo del destinatario de la carta
    	vg_data_sebra(v_count).dire_dest_carta      := i.dire_dest_carta        ;--7  Dirección destino de la carta
    	vg_data_sebra(v_count).ciud_dest_carta      := i.ciud_dest_carta        ;--8  Ciudad del destinatario de la carta
    	vg_data_sebra(v_count).fax_dest_carta       := i.fax_dest_carta         ;--9  Fax destinatario de la carta
    	vg_data_sebra(v_count).tele_dest_carta      := i.tele_dest_carta        ;--10 Telefeno del destinatario de la carta 
    	vg_data_sebra(v_count).emai_dest_carta      := i.emai_dest_carta        ;--11 Correo Electronico del destinatario de la carta 
    	vg_data_sebra(v_count).tipo_oper            := i.tipo_oper              ;--12 Tipo de operación (debito-egreso o credito ingreso)
    	vg_data_sebra(v_count).valor_tras_comi_num  := i.valor_tras_comi_num    ;--13 Valor del traslado mas comisión fija mas comision por millon en números 
    	vg_data_sebra(v_count).valor_tras_comi_let  := i.valor_tras_comi_let    ;--14 Valor del traslado mas comisión fija mas comision por millon en letras  
    	vg_data_sebra(v_count).tipo_cuen            := i.tipo_cuen              ;--15 Tipo de cuenta a la cual se registra el debito y el credito
    	vg_data_sebra(v_count).cod_cuenta           := i.cod_cuenta             ;--16 Cuenta bancaria del aporte
    	vg_data_sebra(v_count).cod_cuent_banc_empr  := i.cod_cuent_banc_empr    ;--17 Cuenta bancaria asociada a la empresa y banco, diferente a la relacionada en el traslado
    	vg_data_sebra(v_count).vari_5               := i.vari_5                 ;--18 Variable 5  
    	vg_data_sebra(v_count).nomb_empr            := i.nomb_empr              ;--19 Nombre de la empresa que hace el traslado
    	vg_data_sebra(v_count).empr_nit             := i.empr_nit               ;--20 Nit de la empresa que hace el traslado
        vg_data_sebra(v_count).concep_cud           := i.concep_cud             ;--21 Concepto CUD 
        vg_data_sebra(v_count).cod_cuen_cud         := i.cod_cuen_cud           ;--22 Código cuenta CUD
        vg_data_sebra(v_count).portafolio           := i.portafolio             ;--23 Portafolio 
        vg_data_sebra(v_count).vari_15              := i.vari_15                ;--24 Variable 15
        vg_data_sebra(v_count).vari_2               := i.vari_2                 ;--25 Variable 2
        vg_data_sebra(v_count).vari_3               := i.vari_3                 ;--26 Variable 3
